...
- jenkins container를 생성 및 구동
    ...
    - `sudo docker run -d -p 8080:8080 -v /home/ubuntu/jenkins-data:/var/jenkins_home --name jenkins jenkins/jenkins:laters`
...
- jenkins 서비스를 구동
    ...
    - jenkins가 재구동될 때 `hudson.model.UpdateCenter.xml`에 설정된 Update Site 경로로부터 플러그인 목록을 받아와 파일을 업데이터하고, 해당 파일을 참조하여 설치 가능한 플러그인 목록을 보여주고 설치 시 URL을 참조
...
- jenkins container 로그 명령어
    - `sudo docekr logs jenkins`
- jenkins container 로그 실시간 확인
    - `sudo docekr logs -f jenkins`
...
- jenkins 웹에서 security 설정이 정상적으로 되어 있는지 확인
    ...
    - Allow anonymous read acces 체크 해제
...
4. 실행 방법 설정(SSH 기준)
    ...
    - **Host Key Verification Strategy**
        - 처음 설정 시에는 `Non-verifying Verification Stratedy`를 선택하면 키 검증 과정을 건너뛸 수 있음
5. 노드 연결 및 확인
...
- Jenkins 설정(빌드 유발 설정)
    ...
    - **Build when a change is pushed to GitLab. GibLab webhook URL:...** 항목 체크(Jenkins에 `GitLab` 플러그인 필요)
        - 이곳에 표시되는 URL을 기록
    - **고급(Advanced...)** 클릭
